  ...
  function renderLeaderboard(byCat) {
    ...
    for (const cat of LB_CATEGORIES) {
      ...
      for (let i = 0; i < pagePlayers.length; i++) {
        ...
        rowsHtml += `<div class="lb-card-row ${rankClass} ${qualityClass}" data-name="${escapeHtml(p.name)}" data-uuid="${escapeHtml(p.uuid || "")}">
...
function drawOverlayAvatarFromDataUrl(ctx, dataUrl, x, y, size) {
  ...
  function drawOverlayBody(ctx, canvas, W, H, name, run, avatarX, avatarY, avatarSize) {
    ...
    if (currentTime != null && best != null) {
      ...
      ctx.fillRect(x - 2, y, 140, 14);
      ctx.strokeStyle = "rgba(255,255,255,0.2)";
      ctx.lineWidth = 1;
      ctx.strokeRect(x - 2, y, 140, 14);

      ctx.fillStyle = textSecondary;
      ctx.font = "7px Inter, sans-serif";
      ctx.fillText(splitLabel + " PB", x, y + 10);

      ctx.fillStyle = textPrimary;
      ctx.font = "bold 8px Inter, sans-serif";
      ctx.fillText(fmt(best), x + 36, y + 10);

      ctx.fillStyle = deltaColor;
      ctx.font = "bold 8px Inter, sans-serif";
      ctx.textAlign = "right";
      ctx.fillText(deltaText, x + 138, y + 10);
      ctx.textAlign = "left";
    }
